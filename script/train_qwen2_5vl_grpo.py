# -*- coding: utf-8 -*-
"""
GRPO RL for Qwen2.5-VL-7B on multi-frame VQA (Python 3.12, T4 16GB)
- Unsloth FastVisionModel (fast_inference=True) => hỗ trợ Qwen2.5-VL
...
- Tùy chọn nạp LoRA SFT 2.5 (nếu bạn đã có), nếu không sẽ RL từ base 2.5

Chạy mẫu:
python scripts/train_qwen25vl_grpo.py \
  --dataset_json data/train/train_with_thinking.json \
  --frames_root data/train_frames_448 \
  --output_dir outputs/qwen25vl_rl \
  --run_name grpo_v2p5_ep1 \
  --epochs 1.0 --lr 5e-6 --num_generations 3 \
  --max_images 3 --short_side 384 --frame_picker uniform
"""
# add at the very top of scripts/train_qwen25vl_grpo.py
import os
...
# ------------------------- Constants -------------------------
BASE_MODEL_25VL = "Qwen/Qwen2.5-VL-7B-Instruct"
...
class RLMultiFrameDataset(Dataset):
    """
    Đọc file JSON (train_with_thinking), tạo prompt đa-ảnh theo chat template của Qwen2.5-VL.
    Trả về:
...
def main():
    ...
    # base + (optional) SFT adapter for Qwen2.5-VL
    parser.add_argument("--base_model", type=str, default=BASE_MODEL_25VL)
    parser.add_argument("--sft_adapter_25", type=str, default="",
                        help="Đường dẫn LoRA SFT Qwen2.5-VL (nếu có). Bỏ trống để RL từ base.")

    # output
    parser.add_argument("--output_dir", type=str, default="outputs/qwen25vl_rl")
    parser.add_argument("--run_name", type=str, default="grpo_v2p5")
    ...
    # reproducibility + CUDA fragmentation mitigation
    random.seed(args.seed)
    torch.manual_seed(args.seed)
    os.environ.setdefault("PYTORCH_CUDA_ALLOC_CONF", "expandable_segments:True")

    # --------- Load Qwen2.5-VL with Unsloth fast path (4-bit)
    model, tokenizer = FastVisionModel.from_pretrained(
        model_name=args.base_model,
        max_seq_length=16384,
        load_in_4bit=True,
        fast_inference=False,           # Hỗ trợ cho Qwen2.5-VL
    ...
    # (Optional) load prior SFT LoRA for Qwen2.5-VL
    if args.sft_adapter_25 and os.path.isdir(args.sft_adapter_25):
        try:
            model.load_adapter(args.sft_adapter_25, adapter_name="sft25", is_trainable=True)
            model.set_adapter("sft25")
            print(f"[OK] Loaded 2.5 SFT adapter: {args.sft_adapter_25}")
        except Exception as e:
            print(f"[WARN] Failed to load 2.5 SFT adapter: {e}")
    ...
    lora_dir = save_dir / "lora_adapter_rl"
