...
python scripts/infer_submit_from_frames.py \
  --test_json data/public_test/public_test.json \
  --frames_root data/test_frames_448 \
  --vl_base Qwen/Qwen2.5-VL-7B-Instruct \
  --vl_adapter outputs/qwen2_5vl7b_unsloth_poc/lora_adapter \
...
def list_frame_paths(frames_root: str, video_path: str, max_images: int = 4) -> List[str]:
    ...
    for ext in ["jpg", "jpeg", "png"]:
        all_paths.extend(glob.glob(os.path.join(frame_dir, f"f*.{ext}")))

    # sort chung tất cả và cắt theo max_images
